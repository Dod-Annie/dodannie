...
const SERVER_URL1 = process.env.baseURL
// 配置 axios，并生成实例
...
// 拦截器配置
creatAxios1.interceptors.request.use(
  configData => {
    // 请求拦截 在发送请求之前做些什么
...
creatAxios1.interceptors.response.use(
  response => {
    // 响应拦截 对响应数据做点什么
    // 响应成功做的事情
...
function sendAll(arr) {
  // 顺序和请求发送的顺序相同，使用 axios.spread 分割成多个单独的响应对象
  if (Object.prototype.toString.call(arr) === '[object Array]') {
    return axios.all(arr).then(
      axios.spread(function(...res) {
        // axios.all 是axios的静态方法，不是实例上的方法
        // 请求全部都执行完成
